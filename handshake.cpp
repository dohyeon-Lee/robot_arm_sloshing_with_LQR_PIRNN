...
{
  string filename = "/home/poweron/2023_UGRP_actuating_code/velocity_test.csv";
...
/* start LQR control with RNN_PINN fluid state estimator */
  VectorXf thetalist_dot(6);
  thetalist_dot << 0,0,0,0,0,0;
  VectorXf before_P_sb(3);
  // for make before_P_sb 
  VectorXf now_thetalist(6);
  actuate_motor.getPosition(groupSyncRead);
  now_thetalist(0) = actuate_motor.motor[0].measured_pos;
  now_thetalist(1) = actuate_motor.motor[1].measured_pos;
  ...
  while (rclcpp::ok()){
    ...
    MatrixXf X_sb = FKinBody(M,Blist,now_thetalist);
    MatrixXf R_sb = X_sb.block<3,3>(0,0);
    VectorXf P_sb = X_sb.block<3,1>(0,3);                     // state 0 
    MatrixXf Jb = JacobianBody(Blist, now_thetalist);
    VectorXf P_dot = (P_sb - before_P_sb)/Duration;           // state 1
    
    accX = node->getReceivedMessage()*9.81;                   // get acc X
    state[0][0][0] = accX;
    std::vector<torch::jit::IValue> inputs;
    ...
    output_tensor = output->elements()[0].toTensor();         // state 2, state 3
    ...
    LQR_state << P_sb[1], P_dot[1], state_theta, state_theta_dot;
